      ...
      <div class="publicPriateSection">
        ...
        <div>
          ...
          >
            ...
            <span
              ><input type="number" name="uniqueNumber" id="uniqueNumber"  v-model="uniqueNumberVal"
            /></span>
          </p> -->
            <!-- <p class="previewForm">
              <button @click="generateLink" class="buttonStyle">
                Generate Link
              </button>
            </p> -->
          </div>
          <!-- <div
            class="uniqueLink"
            v-if="showGenerateLink && formType == 'private'"
          >
            <p class="uniqueTitle">Unique Links</p>
            <ul>
              <li v-for="(uniqueId, index) in uniqueNumberVal" :key="index">
                {{
                  `${title}-${formType}-${formId}-${uniqueNumberVal}${index}${formId}`
                }}
              </li>
            </ul>
        ...
        <div class="forPublicUrl" v-if="formType == 'public' || showPublic">
          <p class="publicUrlInput">
            <input
              ...
              name="publicUrl"
              id="publicUrl"
              :value="`${title}-${formType}-${formId}`"
            />
          </p>
          <p class="previewForm">
            <button class="primarybuttonStyle">Copy Link</button>
          </p>
        </div>
      </div>
    </dashboard-slot>
...
export default {
  ...
  // this.formId=formId
    ...
    sendLink() {
      ...
      //      this.$router.push(
      //        {path: `/sendmail/${this.formId}`, query: {newtitle: newItem }}

      //  )
       this.$router.push(
             {name: 'send-email', params: {newtitle:newItem },fullPath: `/sendmail/${this.formId}`}
...
/* .forPrivateUrl input:nth-child(1) {
    flex: .7;
} */
.uniqueLink ul {
  list-style: circle;
