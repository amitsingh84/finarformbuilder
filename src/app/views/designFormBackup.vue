    ...
    <div v-if="!showHidePreview">
      <div class="desingFormBlockRight">
        <div class="previewForm">
          <button v-if="formLevel" @click="showPrview">Preview</button>
        </div>
        <form-block-right>
         
          <div sytle="position: relative;" v-if="!newElements.length && !formLevel">
            <p class="blankElement">
              Drag your first question here from the right.
            </p>
          </div>
         <div v-if="formLevel"><level-input :formLevel="formLevel" @show-data-id="showLevelSetting"
                  @delete-data-id="deleteElementData"/></div>
          <draggable
            class="content_block"
            :list="newElements"
            item-key="id"
          ...
          >
            ...
            <level-input-setting v-if="emailInputLevel=='Email Address'"  :item="emailInputLevel">sdfsdf</level-input-setting>
    <div v-if="emailInputLevel=='Number'" style="display: block;
    background: yellow;
    width: 30%;
    height: 70px;
    position: absolute;
...
export default {
  ...
  data() {
    return {
      ...
      formLevel:this.$route.params.level,
      emailInputLevel:''
    };
  },
  computed: {
  ...
  // },
    ...
    //    const route=this.$router.push({ name:"show-all-forms",params:{data:this.newElements}}); window.open(route.href, '_blank')
    ...
    showLevelSetting(levelId,items){
console.log(levelId,items);
console.log(items.name);
this.emailInputLevel=items.name
setTimeout(() => {
